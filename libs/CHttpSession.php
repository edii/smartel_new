...
class CHttpSession extends \CApplicationComponent implements IteratorAggregate,ArrayAccess,Countable
{
	...
	public function open()
	{
		...
		if(DEBUG && session_id()=='')
		{
			$message=\init::t('init','Failed to start session.');
			...
			\init::log($message, \CLogger::LEVEL_WARNING, 'system.web.CHttpSession');
		}
	}
	...
	public function setSavePath($value)
	{
		if(is_dir($value))
			...
			throw new \CException(\init::t('init','CHttpSession.savePath "{path}" is not a valid directory.',
				array('{path}'=>$value)));
	}
	...
	public function setCookieMode($value)
	{
		...
		elseif($value==='only')
		{
			...
			throw new \CException(\init::t('init','CHttpSession.cookieMode can only be "none", "allow" or "only".'));
	}
	...
	public function setGCProbability($value)
	{
		if($value>=0 && $value<=100)
		{
			...
			throw new \CException(\init::t('init','CHttpSession.gcProbability "{value}" is invalid. It must be a float between 0 and 100.',
				array('{value}'=>$value)));
	}
