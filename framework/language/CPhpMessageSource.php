...
class CPhpMessageSource extends \CMessageSource {
	const CACHE_KEY_PREFIX='init.CPhpMessageSource.';
	...
	public function init()
	{
		...
		if($this->basePath===null)
			$this->basePath=\init::getPathOfAlias('application.messages');
	}
		...
		if(!isset($this->_files[$category][$language]))
		{
			if(($pos=strpos($category,'.'))!==false)
			{
				...
				if(isset($this->extensionPaths[$extensionClass]))
					$this->_files[$category][$language]=\init::getPathOfAlias($this->extensionPaths[$extensionClass]).DIRECTORY_SEPARATOR.$language.DIRECTORY_SEPARATOR.$extensionCategory.'.php';
				else
				{
	...
	protected function loadMessages($category,$language)
	{
		...
		if($this->cachingDuration>0 && $this->cacheID!==false && ($cache=\init::app()->getComponent($this->cacheID))!==null)
		{
